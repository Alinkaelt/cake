...
\begin{proof}
\textcolor{white}{x}\\\\
For showing that lone divider is $notGTCCSP$ it is necessary to have at least one cheating player $p_c$. This player has a non-truthful strategy with $v_c(X_c)>v_c(X_t)$ in one allocation and $v_c(X_c) \geq v_c(X_t)$ for all other allocations. The case $v_c(X_c)>v_c(X_t)$ can be illustrated with an example. 

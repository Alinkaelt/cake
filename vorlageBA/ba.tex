...
A \emph{best response} is a strategy (pure or mixed) which maximizes the player's expected utility given the strategies of all the other agents.\\ 
A dominant strategy is a strategy that is a best response regardless of the strategies of the other players.
}
\end{defi}
...
\emph{In a \emph{zero-sum game} the sum of the valuations of an allocation is one. Otherwise the game is called \emph{general-sum game}.}
\end{defi}
...
\begin{lem}
\label{thm5} The relation between different strategyproofness criteria is the following $$SPP \impl{1} SP \impl{2} WSP$$ and
$$GTSP \impl{3} SP.$$
...
\begin{center}
	\begin{tikzpicture}
		...
		\node[draw=none,fill=none] (n5) at (1.8,5.3) {$1$};
		\node[draw=none,fill=none] (n6) at (3.1,5.3) {$2$};
		\node[draw=none,fill=none] (n7) at (2.7,4.4) {$3$};
